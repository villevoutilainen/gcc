			...
			const struct cl_option_handlers *handlers)
{
  ...
  switch (code)
    ...
    case OPT_fprofiles_enforce_:
      /* Comma-separated profile list; split (and validated) once, by
	 handle_profiles_enforce_option, after option processing --
	 see that function's own comment.  */
      defer_opt (code, arg);
      break;
...
/* Split every deferred -fprofiles-enforce=name[,name...] occurrence
   (see this file's own OPT_fprofiles_enforce_ case, above) on commas,
...
static void
handle_profiles_enforce_option (void)
{
  for (unsigned i = 0; i < deferred_count; i++)
    {
      struct deferred_opt *opt = &deferred_opts[i];
      if (opt->code != OPT_fprofiles_enforce_)
	continue;
      ...
      while (true)
	{
	  ...
	  if (len == 0)
	    {
	      error ("empty profile name in %<-fprofiles-enforce=%s%>", arg);
	      break;
	    }
