// D4324: same mechanism as d4324-conveyor-proof-float-field-range-ok.C,
// genuine violation -- the field's established range [40.0,100.0) is
// disjoint from consume_value's own required range (>= 200.0).
// { dg-do compile { target c++26 } }
// { dg-additional-options "-fcontracts -fcontract-control-objects -fcontract-conveyor-proofs" }

#include <contracts>
namespace sc = std::contracts;

struct conveyor_ctrl {
  static constexpr bool is_ignored (sc::assertion_static_info) { return false; }
  static constexpr bool constify (sc::assertion_static_info) { return false; }
  static constexpr bool assumable (sc::assertion_static_info) { return false; }
  static constexpr bool is_conveyor (sc::assertion_static_info) { return true; }
  void operator() (const sc::assertion_context& ctx) const
  { if (!ctx.check ()) __builtin_trap (); }
};
inline constexpr conveyor_ctrl conveyor_ctrl_v{};

struct thing {
  double value;
  void produce_value ()
    post<conveyor_ctrl_v>(this->value >= 40.0 // { dg-warning "cannot verify postcondition" }
			  && this->value < 100.0) // { dg-warning "cannot verify postcondition" }
  { value = 55.0; }
  void consume_value ()
    pre<conveyor_ctrl_v>(this->value >= 200.0)
  { }
};

int
main ()
{
  thing t;
  t.produce_value ();
  t.consume_value (); // { dg-error "provably violates the precondition" }
  return 0;
}
