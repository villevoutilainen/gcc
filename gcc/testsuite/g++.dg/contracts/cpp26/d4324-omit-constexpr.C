...
inline constexpr probe_keeps_location probe_keeps_location_v{};
...
constexpr int fkl (int x) pre<probe_keeps_location_v>(x >= 0) { return x; }
...
constexpr unsigned
observe_kept_location_line ()
{
  try { fkl (-1); } catch (unsigned l) { return l; }
...
static_assert (observe_kept_location_line () != 0);
