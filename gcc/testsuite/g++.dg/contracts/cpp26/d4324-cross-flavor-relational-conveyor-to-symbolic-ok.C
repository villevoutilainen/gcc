...
// relational facts (oa_relational_fact's own conveyor_established tag)
// -- see d4324-cross-flavor-relational-symbolic-to-conveyor-unknown.C
// for the forbidden direction. g_conveyor's own CONVEYOR-flavored
// precondition establishes "x < q" (self-trust); f_symbolic's own
// SYMBOLIC-flavored precondition requires the same relation on the
// same forwarded pair -- a conveyor-established fact is trustworthy
// enough for symbolic's own check to rely on, so this is discharged
// silently. See .claude/plans/well-we-last-discussed-ethereal-
// duckling.md.
